...
class MultiModalTemporalPlanner(nn.Module):
    ...
    def __init__(
        ...
        time_aligned_bias_init: float = 5.0,
    ):
        super().__init__()
        ...
        # ── 特征投影（逐 token 投影，与 Drive-JEPA 对齐）─────────────
        self.token_proj = nn.Linear(encoder_dim, tf_d_model)

        # ── Positional encoding ───────────────────────────────────────
        # use_spatial_tokens: 保留每帧 P 个 token → P*T + 1
        # 否则: 每帧池化为 1 个 token → T + 1
        ...
        # ── 时间对齐 bias（可学习强度标量） ────────────────────────────
        if use_time_aligned_bias:
            self.time_aligned_bias_logstrength = nn.Parameter(
                torch.tensor(math.log(time_aligned_bias_init))
            )
    ...
    def _build_time_aligned_memory_bias(
        ...
        """构建时间对齐 additive bias [Q, M]，加到 decoder cross-attn logit 上。

        strength 通过 exp(logstrength) 保证为正。模型自行学习合适的惩罚强度。
        memory_step_idx 中 -1 标记的 token（status）不受距离惩罚。
        """
        strength = self.time_aligned_bias_logstrength.exp()
        ...
        bias = -strength * (distance / norm)                 # [Q, M]
    ...
    def _build_memory(
        ...
        # 逐 token 投影: [B, T, P, D] → [B, T, P, d]
        feat_projected = self.token_proj(z_ar_reshaped)

        if self.use_spatial_tokens:
            # 保留空间 token: [B, T, P, d] → [B, T*P, d]
            feat = feat_projected.view(B, T * P, -1)
            memory_step_idx = torch.arange(T, device=z_ar.device, dtype=torch.long).repeat_interleave(P)
        else:
            # 空间维度平均池化: [B, T, P, d] → [B, T, d]
            feat = feat_projected.mean(dim=2)
        ...
        # Status token (step_idx = -1, 不参与时间距离惩罚)
    ...
    def forward(
        ...
        # 2. 时间对齐 bias（可学习强度）
        memory_bias = None
        if self.use_time_aligned_bias:
            memory_bias = self._build_time_aligned_memory_bias(
                self.query_step_idx, memory_step_idx, dtype=memory.dtype,
            )  # [Q, M]
        ...
        if memory_bias is not None:
            ...
            query_out = self.transformer(src=memory, tgt=query)       # [B, K*num_poses, d]
        query_out = query_out.view(B, K, self.num_poses, self.tf_d_model)

        # 4. Per-mode trajectory regression
        traj_list = []
        for k in range(K):
        ...
        # 5. Confidence (pool over poses dim → [B, K, d] → [B, K*d] → [B, K])
...
def main(args, resume_preempt=False):
    ...
    planner_grad_to_encoder = cfgs_planner.get("planner_grad_to_encoder", False)
    use_time_aligned_bias = cfgs_planner.get("use_time_aligned_bias", True)
    time_aligned_bias_init = cfgs_planner.get("time_aligned_bias_init", 5.0)
    # -- DATA
    ...
    if use_planner:
        planner = MultiModalTemporalPlanner(
            ...
            time_aligned_bias_init=time_aligned_bias_init,
        ).to(device)

        planner_params = sum(p.numel() for p in planner.parameters())
        logger.info(
            f"planner_params: {planner_params / 1e6:.2f}M "
            f"(TemporalPlanner, num_time_steps={num_time_steps}, "
            f"use_spatial_tokens={use_spatial_tokens}, "
            f"use_time_aligned_bias={use_time_aligned_bias}, "
            f"bias_init={time_aligned_bias_init})"
        )
    else:
        planner = None
    ...
    # 添加 planner 参数（区分 bias/non-bias，与其他模块保持一致）
    if use_planner:
    ...
    # 只有当 encoder_train=True 时才添加（区分 bias/non-bias，与其他模块保持一致）
    if encoder_train:
        optimizer.add_param_group({
    ...
    if p_file_full is not None and os.path.exists(p_file_full):
        ...
        if load_planner and use_planner and planner is not None and 'planner' in checkpoint:
            ...
            def train_step():
                ...
                with torch.cuda.amp.autocast(dtype=dtype, enabled=mixed_precision):
                    ...
                    if use_planner and planner is not None:
                        ...
                        if planner_grad_to_encoder and encoder_train:
                            z_ar_planner = z_ar
                        else:
                            z_ar_planner = z_ar.detach()
