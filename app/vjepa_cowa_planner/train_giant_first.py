    ...
    def forward(
        ...
        if self.use_z_context:
            assert z_context is not None, (
                ...
                "Please pass z_context (encoder output) to planner.forward()."
            )
        ...
        if self.use_temporal:
            ...
            # bias 矩阵会被加到 decoder cross-attention 的 attention score 上
            # 效果：让第 i 个 query 更关注第 i 个时间步的 memory token
...
def main(args, resume_preempt=False):
    ...
    causal_mask = cfgs_planner.get("causal_mask", False)  # 是否使用 causal mask
